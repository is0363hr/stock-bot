...
def scraping():
    ...
    for num in range(0,40):
        ...
        if len(stockLine)==302:
            ...
            print(stockETFPriceAfter[6].text)
    ...
    table.to_csv("./data/stockPriceData_ETF.csv",index=False, encoding='utf_8_sig');
    ...
    df_date2["date"]=dates
    df_date2["year"]=df_date2["date"].apply(lambda x:int(x.split("-")[0]))
    df_date2["month"]=df_date2["date"].apply(lambda x:int(x.split("-")[1]))
    df_date2["day"]=df_date2["date"].apply(lambda x:int(x.split("-")[2]))
    df_date2.head()
    ...
    df_next.to_csv("./label/nomura.csv",index=False, encoding='utf_8_sig');